...
class Agent():
    ...
    def learn(self):
        ...
        done = 1.0 - torch.FloatTensor(done)
        ...
        q_current = evaluate_Q[batch_indices, actions]
        target_futureQ = torch.max(target_Q, dim=1)[0]
                
        q_target = rewards + self.gamma * target_futureQ * done
        ...
        current_process = self.buffer.memory[-1]
    ...
    def check_max_Q(self):
        ...
        max_q = torch.max(self.target_net.forward(torch.FloatTensor(initial_state)))
        return max_q
        # End your code
...
def train(env):
    ...
    #to check if training is finish
    global EPISODE
    EPISODE = episode
...
if __name__ == "__main__":
    ...
    if os.path.exists("./Tables/DQN.pt"):
        os.remove("./Tables/DQN.pt")
    # training section:
    for i in range(5):
