...
class Agent():
    ...
    def init_bins(self, lower_bound, upper_bound, num_bins):
        ...
        """
        # Begin your code
        bins = np.linspace(lower_bound, upper_bound, num_bins+1)[1:-1]
        return bins
        # End your code
    ...
    def learn(self, state, action, reward, next_state, done):
        ...
        best_qvalue = np.max(self.qtable[tuple(next_state)])
        current_qvalue = self.qtable[tuple(state) + (action, )]
        if not done:
            self.qtable[tuple(state) + (action, )] += self.learning_rate * (reward + self.gamma * best_qvalue - current_qvalue) 
        elif done:
            self.qtable[tuple(state) + (action, )] += self.learning_rate * (reward - current_qvalue)
        # End your code
    ...
    def check_max_Q(self):
        ...
        """
        # Begin your code
        state = tuple(self.discretize_observation(self.env.reset()))
        max_q = np.amax(self.qtable[tuple(state)])
        
        # best reward: 200 (200 steps)
        # 1 + 1*g + 1*g^2 + ... + 1*g^199
        power = np.power(self.gamma, 200)
        opt_Q = (1 - power) / (1 - self.gamma)
...
if __name__ == "__main__":
   ...
   # training section:
    if os.path.exists("./Tables/cartpole_table.npy"):
        os.remove("./Tables/cartpole_table.npy")
