...
class ExactInference(object):
    ...
    """
    Explain:
        Use a two layer for loop to update probabilty of all positions
        Update the probabilty with original probability multiplied with likelihood, which is calculated using
            probabilty density function of Gaussian with mean (distance of position of agent and (x, y))
    ...
    def observe(self, agentX: int, agentY: int, observedDist: float) -> None:
        ...
        for row in range(self.belief.getNumRows()):
            for col in range(self.belief.getNumCols()):
                ...
                likelihood = util.pdf(math.sqrt( pow(agentX-x, 2) + pow(agentY-y, 2) ), Const.SONAR_STD, observedDist)
                new_prob = prev_prob * likelihood
    ...
    """
    Explain:
        Initialize new_belief to all 0
        For all transition:
            Add (transition probability * old_belief) to new_belief(initial 0) at the position after transition  
        Normalize new_belief and update belief by assigning new_belief to self.belief 
    """
    def elapseTime(self) -> None:
        ...
        for (oldTile, newTile) in self.transProb:
            belief_prob = self.belief.getProb(oldTile[0], oldTile[1])
            transition = self.transProb[(oldTile, newTile)]
            delta_prob = transition * belief_prob
            new_belief.addProb(newTile[0], newTile[1], delta_prob)
...
class ParticleFilter(object):
    ...
    """
    Explain:
        Quite similar to part 1, update the belief with the particles
        Update the probabilty with original probability multiplied with likelihood, which is calculated using
            probabilty density function of Gaussian with mean (distance of position of agent and (x, y)) 
        Re-weight the particles by the observation
        Re-sample particles from the reweighted distribution and create NUM_PARTICLES new particles
        Normalize and update belief through self.updateBelief()
    """
    def observe(self, agentX: int, agentY: int, observedDist: float) -> None:
        ...
        for tile in self.particles.keys():
            ...
            prev_prob = self.particles[tile]
            likelihood = util.pdf(math.sqrt(pow(agentX-x, 2) + pow(agentY-y, 2)), Const.SONAR_STD, observedDist)
            self.particles[tile] = prev_prob * likelihood
        
        new_particles = collections.Counter()

        for _ in range(self.NUM_PARTICLES):
            sample = util.weightedRandomChoice(self.particles)
            new_particles[sample] += 1
    ...
    """
    Explain:
        Sample new particle to see where it would land by transition probability
        If on that tile there're more particles, that tile would be more important, which may 
            have more landing positions
        Update new_particles to self.particles   
    """
    def elapseTime(self) -> None:
