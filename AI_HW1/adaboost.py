...
class Adaboost:
    ...
    def selectBest(self, featureVals, iis, labels, features, weights):
        ...
        '''
        Implement the classify feature part 
          h(x) = 1 if featureVal < 0
                 0 if featureVal >= 0

        Calculate the error of each classifier
          error = sum (weight * |h(x) - label|)
          
        Find the minimum of the errors to decide the best classifier
        Construct a WeakClassifier class with the min error feature
        Return best error and best classifier
        '''
        ...
        for fv in featureVals:
          ...
          for i in range(len(fv)):
            error += weights[i] * abs((fv[i] < 0) - labels[i])
          errors.append(error)
        
        bestError = min(errors)
        bestClf = WeakClassifier(features[errors.index(bestError)])
        ...
        for fv in featureVals:
          error = 0
          for i in range(len(fv)):
            error += (weights[i] * abs((fv[i] < 0) - labels[i])) ** 2
          error /= len(fv)
